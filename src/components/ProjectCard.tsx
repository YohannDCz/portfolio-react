...
// ——————————————————————————————————————————————
// PROJECT CARD COMPONENT WITH RTL SUPPORT
// ——————————————————————————————————————————————

/**
 * Project card component displaying project information with admin controls
 * @param props - Project card properties
 * @returns JSX Element for project card
 */
function ProjectCard({ project, currentLang, t, isAdminMode = false }: ProjectCardProps): JSX.Element {
      ...
      <Card className="h-full relative overflow-hidden">
        ...
        {project.status === 'in_progress' && (
          <div className="absolute top-2 left-2 z-10">
            <Badge
              variant="outline"
              className="text-xs bg-yellow-100 text-yellow-800 border-yellow-300 dark:bg-yellow-900/50 dark:text-yellow-300 dark:border-yellow-700"
            >
              {t.inProgress}
            </Badge>
          </div>
        ...
        {project.status === 'to_deploy' && (
          <div className="absolute top-2 left-2 z-10">
            <Badge
              variant="outline"
              className="text-xs bg-green-100 text-green-800 border-green-300 dark:bg-green-900/50 dark:text-green-300 dark:border-green-700"
            >
              {t.toDeploy}
            </Badge>
          </div>
        ...
        <CardHeader className="pb-2">
          <div className={`${getDirectionalClass('flex items-start justify-between')} gap-2`}>
            <div className="flex-1">
              ...
              <CardDescription className="line-clamp-2 min-h-[2.5rem] mb-2">
        ...
        <CardContent className="flex flex-col">
          <div
            className={`${getDirectionalClass('flex flex-wrap gap-1.5')} mb-4 content-start flex-1`}
          >
            {/* Project categories */}
            {project.category && (Array.isArray(project.category) ? project.category : [project.category]).map(
              (cat) => (
                <Badge
                  key={`${project.id}-category-${cat}`}
                  variant="default"
                  className="rounded-full capitalize h-6 px-2.5 text-xs flex-shrink-0"
                >
                  {cat}
                </Badge>
              ),
            ...
            {project.technologies?.slice(0, 8).map((tag) => (
              <Badge
                key={`${project.id}-tech-${tag}`}
                variant="secondary"
                className="rounded-full h-6 px-2.5 text-xs flex-shrink-0 max-w-[120px] truncate"
              >
                {tag}
              </Badge>
            ))}
          ...
          <div className={`${getDirectionalClass('flex gap-2')} mt-auto pt-3 border-t`}>
            ...
            <Link
              href={project.demo_url && project.demo_url !== '#' ? project.demo_url : '#'}
              target={project.demo_url && project.demo_url !== '#' ? '_blank' : '_self'}
            ...
            >
              <Button
                ...
                disabled={!project.demo_url || project.demo_url === '#'}
              >
            ...
            {/* GitHub icon - only when repo_url exists */}
            {project.repo_url && (
